...
5. Join the stores data with the sales data.

```{r, eval=FALSE, echo=FALSE}
sales <- left_join(sales, stores, by = "Store")
```

6. The sales data contains an error in the state_holiday variable. There are `NA`s where there should be `1`s. Change this (remember that `variable == NA` is not an appropriate logical expression, use `is.na(variable)` instead).
...
8. Save the prepared data as "retailer_sales.csv".
...
### Do a similar plot using ggplot:
