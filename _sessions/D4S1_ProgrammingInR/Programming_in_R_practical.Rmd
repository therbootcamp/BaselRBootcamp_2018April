...
# Create coin toss, i.e. sample once from {H, T} without replacing
coin_toss <- sample(c("H", "T"), 1)

# In 1 % of the cases insert NULL
if (runif(1, 0, 1) <= .01){ # You can use expressions / function calls 
                            # in an if statement
  coin_toss <- NULL
}
```
...
4. Now you don't want to toss your coin only once but several (let's say ten) times (you'll have to tweak the `sample` function a bit. Check the help menue for help). Do this, and if *ALL* tosses came up heads, print "Wow, that's ten heads in a row!", if there is at least one head in there, print "The coin showed X times heads, and Y times tails", where X and Y are the respective numbers of heads and tails (use the `paste` function to print and the `sum` function to get the correct numbers, check the help menus if you don't know how to do this), and print "Wow, that's ten tails in a row!", if *ALL* tosses came up tails. This coin has a very thin edge, so you can ignore the possibility of it landing and staying on the edge.
...
# or all()
all(coin_tosses_1 == coin_tosses_2)
...
# so the correct answer is
if (all(coin_tosses_1 == coin_tosses_2)){
  print("They are identical")
}
```
...
6. Write your own function `check_logical` that tests whether a given logical object is `TRUE` or `FALSE`, and returns "Argument is TRUE", and "Argument is FALSE", respectively. 
