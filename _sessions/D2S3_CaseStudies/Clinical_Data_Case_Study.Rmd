...
dimarta_trial$histamine_end[dimarta_trial$arm == "I"] <- dimarta_trial$histamine_start[dimarta_trial$arm == "I"] + round(rnorm(sum(dimarta_trial$arm == "I"), mean = 3, sd = 5), 2)
dimarta_trial$histamine_end[dimarta_trial$arm == "II"] <- dimarta_trial$histamine_start[dimarta_trial$arm == "II"] + round(rnorm(sum(dimarta_trial$arm == "II"), mean = 1, sd = 5), 2)
dimarta_trial$histamine_end[dimarta_trial$arm == "III"] <- dimarta_trial$histamine_start[dimarta_trial$arm == "III"] + round(rnorm(sum(dimarta_trial$arm == "III"), mean = 2, sd = 5), 2)
...
dimarta_trial$histamine_start[sample(nrow(dimarta_trial), size = 5)] <- c(999, -24, 888, -99, 999)
dimarta_trial$histamine_end[sample(nrow(dimarta_trial), size = 10)] <- NA
dimarta_trial$histamine_start[sample(nrow(dimarta_trial), size = 7)] <- NA
...
4. Get a first impression of the datasets by exploring them with the `View()`, `head()`, `names()`, and `str()` functions. Were they all loaded correctly?
...
10. Now, using the `left_join` function, add the biomarker results to the `dimarta_df` data! Now you should hve all of the data in a single dataframe called `dimarta_df`

11. View `dimarta_df` to make sure the data look correct.

12. Using `write_csv()`, save `dimarta_df` in a new `.csv` file on your computer called `dimarta.csv`
