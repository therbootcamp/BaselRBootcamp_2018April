...
You'll use two datasets in this practical: `heartdisease.csv`, and `ACTG175.csv`. They are available in the .zip file at https://github.com/therbootcamp/BaselRBootcamp_2018April/blob/master/_sessions/_data/data_BaselRBootcamp_Day2.zip?raw=true. Open the link in a browser to install the .zip folder. Unzip the folder (by double clicking it). When you do, you should seee the two files!
...
A. Load all of the necessary packages. For this practical, we'll need `FFTrees`, `e1071`, `randomForest`, and `tidyverse`. 

#### Part B: Create training and test data

B. Move the `heartdisease.csv` file to a folder called `data` in your working directory. Then, using `read_csv()`, read the `heartdisease.csv` data and assign it to a new object in R called `heartdisease`. 

C. Explore the `heart` dataset using `summary()`, and `View()`

D. Create separate training dataframes `heart_train` for model training and `heart_test` for model testing. Print each of these dataframes to make sure they look ok.
...
11. So far we've only looked at the `heartdisease` data. Try conducting a similar analysis on the `ACTG175` data. Specifically, try creating models predicting whether or not the number of days until the first occurence of a major negative event is less than 1,000 days. Which of the different machine learning algorithms performs the best in predicting new data from this dataset? Do you discover any challenges in working with dataset that weren't present in the `heartdisease` data?
