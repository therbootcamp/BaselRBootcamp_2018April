...
- Given a dataset, the majority of machine learning models are evaluated based on 10-Fold cross validation.
...
- Caret is data scientist's dream toolbox for conducting machine learning.

- Caret has functions that allow you to do most machine learning tasks with a few lines of code.
...
```{r, echo = FALSE, fig.align = 'center', out.width = "80%", fig.cap = "The almighty Caret!"}
...
- Use `train()` to fit a model and get best parameters
...
See all models at [http://topepo.github.io/caret/available-models.html](http://topepo.github.io/caret/available-models.html)
...
- There are *many* other great features of caret we haven't touched
   ...
   - Imputing (replacing) missing values with `preProcess()`
   - Transforming predictors with `preProcess()`
