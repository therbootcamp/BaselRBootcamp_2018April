...
```{r, echo = FALSE}
knitr::opts_chunk$set(comment=NA, fig.width=6, fig.height=4, echo = TRUE, eval = TRUE, fig.align = 'center')
```
...
```{r, eval = FALSE}
# Just change all formulas so that diagnosis is the criterion
# For example....
...
```{r, eval = FALSE}
# Change value of truth_train to diagnosis
truth_train <- heartdisease$diagnosis
```

*Here's the plot I got for training accuracy. It looks like random forests are almost perfect!*
...
*Here's what I got, in the test data, all three models are pretty much the same*
...
randomForest_10_fit <- predict(object = randomForest_10_model, 
                            newdata = heart_train)

randomForest_10000_fit <- predict(object = randomForest_10000_model, 
                            newdata = heart_train)
...
# Put training results together
training_results <- tibble(model = c("FFTrees", "glm", "randomForest", "rf10", "rf10000"),
                          result = c(mean(FFTrees_fit == train_truth),
                                     mean(glm_fit == train_truth),
                                     mean(randomForest_fit == train_truth),
                                     mean(randomForest_10_fit == train_truth),
                                     mean(randomForest_10000_fit == train_truth)))
...
testing_results <- tibble(model = c("FFTrees", "glm", "randomForest", "rf10", "rf10000"),
                          result = c(mean(FFTrees_pred == test_truth),
                                     mean(glm_pred == test_truth),
                                     mean(randomForest_pred == test_truth),
                                     mean(randomForest_10_pred == test_truth),
...
# randomForest training decisions
svm_fit <- predict(object = svm_model, 
                  newdata = heart_train)
...
# Put training results together
training_results <- tibble(model = c("FFTrees", "glm", "randomForest", "svm"),
                          result = c(mean(FFTrees_fit == train_truth),
                                     mean(glm_fit == train_truth),
                                     mean(randomForest_fit == train_truth),
                                     mean(svm_fit == train_truth)))
...
```{r}
# Try on your own, you should find that the more complex models have above chance fitting performance, but all models have change level prediction performance
```
...
```{r}
# Try on your own, you should find similar results as our previous analyses
```
